...
Optional, domain-agnostic external metadata enrichment for `SearchableItem`. Operators want to see reference metadata for tracked items — e.g. a Magic: the Gathering card's image and description pulled from Scryfall — without turning `SearchableItem` or its templates into an MTG-specific data model. Since the project tracks non-card items (coffee, computer parts) alongside MTG singles, this capability generalizes across domains: provider selection, resolution/disambiguation, a generic three-slot display contract, staleness/re-fetch triggers, a bounded-rate refresh queue, and the associated UI surfaces (forms, detail page, list thumbnail). Metadata enrichment is purely additive and display-only — it never influences vendor search-result relevance filtering.
