...
`item-category-relevance-filter` adds two further, independent checks at the same `add_result` choke point: `expected_product_line` (checked against `products[].vendor`, the vendor's broad game/product-line signal) and `expected_category` (checked against `productLineData.set`, the existing narrow set-level signal). `search_results_product_line_mismatch.json` is a small supplementary fixture covering both: a same-titled `"Lightning Bolt"` product from an unrelated product line (Disney Lorcana), and a same-product-line product from an unrelated set (Masters 25 vs. an expected Strixhaven). If `search_results_sample.json` itself is refreshed, ideally include at least one off-product-line row and one off-category row there too for regression coverage, mirroring this note. See `hfx_investigation.md` for the naming-collision callout between this `product_line` (response-side field/column) and the unrelated `product_line` Storepass **query parameter** used in `base_search_url` above.
